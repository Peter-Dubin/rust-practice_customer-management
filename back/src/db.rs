...
use crate::models::{
    CreateCustomerRequest, CreateSupplierRequest, Customer, PaginatedResponse, Supplier,
...
impl Db {
    ...
    pub fn list_suppliers(
        ...
        let allowed_columns = ["CompanyName", "ContactName", "City", "Country", "SupplierID"];
        let order_col = order_by
            .filter(|c| allowed_columns.contains(c))
            .unwrap_or("CompanyName");
        let order_dir = match order_direction {
        ...
        let (count_sql, list_sql) = if filter_val.is_some() {
            (
                "SELECT COUNT(*) FROM Suppliers WHERE CompanyName LIKE ?1".to_string(),
                format!(
                    "SELECT SupplierID, CompanyName, ContactName, ContactTitle, \
                     Address, City, Region, PostalCode, Country, Phone, Fax, HomePage \
                     FROM Suppliers WHERE CompanyName LIKE ?1 \
                     ORDER BY {} {} LIMIT ?2 OFFSET ?3",
                    order_col, order_dir
                ),
            )
            ...
            (
                "SELECT COUNT(*) FROM Suppliers".to_string(),
                format!(
                    "SELECT SupplierID, CompanyName, ContactName, ContactTitle, \
                     Address, City, Region, PostalCode, Country, Phone, Fax, HomePage \
                     FROM Suppliers ORDER BY {} {} LIMIT ?1 OFFSET ?2",
                    order_col, order_dir
                ),
            )
        };
        ...
        let mut stmt = conn.prepare(&list_sql)?;
        let map_row = |r: &rusqlite::Row| -> rusqlite::Result<Supplier> {
            Ok(Supplier {
                supplier_id: r.get(0)?,
                company_name: r.get(1)?,
                contact_name: r.get(2)?,
        ...
        let suppliers: Vec<Supplier> = if let Some(ref fv) = filter_val {
            stmt.query_map(params![fv, per_page, offset], map_row)?
                .collect::<Result<Vec<_>>>()?
        } else {
            stmt.query_map(params![per_page, offset], map_row)?
    ...
    pub fn get_supplier(&self, id: i32) -> Result<Option<Supplier>> {
        let conn = self.0.lock().unwrap();
        let sql = "SELECT SupplierID, CompanyName, ContactName, ContactTitle, \
                   Address, City, Region, PostalCode, Country, Phone, Fax, HomePage \
    ...
    pub fn create_supplier(&self, req: &CreateSupplierRequest) -> Result<Supplier> {
        let conn = self.0.lock().unwrap();
        let new_id: i32 = conn.query_row(
            "SELECT COALESCE(MAX(SupplierID), 0) + 1 FROM Suppliers",
            [],
            |r| r.get(0),
        )?;
        conn.execute(
        ...
        Ok(Supplier {
            ...
            phone: req.phone.clone(),
            fax: req.fax.clone(),
            home_page: req.home_page.clone(),
        })
    }

    pub fn update_supplier(&self, id: i32, req: &UpdateSupplierRequest) -> Result<bool> {
        let conn = self.0.lock().unwrap();
        let rows = conn.execute(
            "UPDATE Suppliers SET \
             CompanyName=?1, ContactName=?2, ContactTitle=?3, Address=?4, \
             City=?5, Region=?6, PostalCode=?7, Country=?8, Phone=?9, Fax=?10, HomePage=?11 \
             WHERE SupplierID=?12",
            params![
                req.company_name,
                req.contact_name,
    ...
    pub fn delete_supplier(&self, id: i32) -> Result<bool> {
        let conn = self.0.lock().unwrap();
        let rows = conn.execute(
